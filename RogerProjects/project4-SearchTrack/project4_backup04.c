...
int record_BallPos(Robot* roger, double time, Observation* obs, BallPos* bps) 
{
	int head = bps->head;
	int tail = bps->tail;
	int size = bps->size;
	...
	double x = obs->pos[X];
	double y = obs->pos[Y];
	if (time > 0.001) { // avoid record MAX_RAND at the beginning
	...
	double epsilon = 5.5;
	if (prev_x < MAX_RAND && diff_x > epsilon) {
		x = MAX_RAND;
	}
	if (prev_y < MAX_RAND && diff_y > epsilon) {
		y = MAX_RAND;
	}
...
double time;
{
	...
	if ( average_red_pixel(roger, &ul, &ur) == TRUE ) {
		...
		// record_BallPos(roger, time, &obs, &bps);
		// double vel_x;
		// double vel_y;
		// velocity_Ball(roger, time, &bps, &vel_x, &vel_y);
