...
double time;
{
	...
	***/
	...
	// 	printf("SEARCHTRACK(): CONVERGED\n");
		...
		recommended_setpoints[R_CHASE][X] = obs.pos[X];
		recommended_setpoints[R_CHASE][Y] = obs.pos[Y];
...
double time;
{
	...
	TOUCH()
	{
		...
		if( left reach ) {
			arm_left_setpoint = theta1, theta2
		}
	...
	***/
	...
	printf("left_reach:%d  right_reach:%d\n", left_reach, right_reach);
...
double time;
{
	...
	double home_2[2][2] = {{2.7, -2.7}, {-2.7, 2.7}};
	switch (state) {
						// TOUCH 					CHASE
		...
		case 5: // TRANSIENT - CONVERGED
			roger->base_setpoint[X] = recommended_setpoints[R_CHASE][X];
			roger->base_setpoint[Y] = recommended_setpoints[R_CHASE][Y];
			roger->arm_setpoint[LEFT][0] = recommended_setpoints[R_TOUCH][LEFT_JOINT_0];
			roger->arm_setpoint[LEFT][1] = recommended_setpoints[R_TOUCH][LEFT_JOINT_1];
			roger->arm_setpoint[RIGHT][0] = recommended_setpoints[R_TOUCH][RIGHT_JOINT_0];
			roger->arm_setpoint[RIGHT][1] = recommended_setpoints[R_TOUCH][RIGHT_JOINT_1];
			...
			// printf("Chase:%d  Touch:%d Case 3 4 5\n", internal_state[0], internal_state[1]);
			return_state = TRANSIENT;
		break;
		...
		case 8: // CONVERGED - CONVERGED
			...
			// roger->eyes_setpoint[LEFT] = recommended_setpoints[1][LEFT];
			// roger->eyes_setpoint[RIGHT] = recommended_setpoints[1][RIGHT];
			// printf("Chase:%d  Touch:%d Case 6 7 8\n", internal_state[0], internal_state[1]);
