...
int record_BallPos(Robot* roger, double time, SetPoints* rsp, Observation* obs, BallPos* bps) 
{
	...
	printf("head:%d  tail:%d  size:%d  cnt:%d\n", head, tail, size, cnt);
	printf("record_BallPos1\n");

	double x = obs->pos[X];
	double y = obs->pos[Y];
	printf("record_BallPos2\n");
	...
	printf("record_BallPos7  head:%d  tail:%d\n", head, tail);
	bps->time[tail] = time;
	bps->x[tail] = x;
	bps->y[tail] = y;
	bps->tail = tail;
	bps->head = head;
	printf("record_BallPos8\n");
	if (cnt < NBINS_BPS) {
		bps->cnt++;
	}
	printf("record_BallPos9\n");
...
int velocity_Ball(Robot* roger, double time, BallPos* bps, double* vel_x, double* vel_y)
{
	...
	*vel_x = (bps->x[tail] - bps->x[head]) / (NBINS_BPS+0.00000000001);
	*vel_y = (bps->y[tail] - bps->y[head]) / (NBINS_BPS+0.00000000001);
}
...
double time;
{
	...
	printf("head:%d  tail:%d  size:%d  cnt:%d\n", bps_global.head, bps_global.tail, bps_global.size, bps_global.cnt);
