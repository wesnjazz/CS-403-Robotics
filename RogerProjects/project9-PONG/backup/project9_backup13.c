...
int OBSERVEBALL(Robot* roger, double time, SetPoints* rsp, Observation* obs, BallPos* bps)
{
	if (SEARCHTRACK(roger, time) == CONVERGED) {
		...
		if ( tiktok_observeball % tiktok_observeball_interval == 0)  {
			// if (cnt >= NBINS_BPS) {
			// 	while (sum_cnt < cnt) {
			// 		sum_x += (bps->x[head+1] - bps->x[head]);
			// 		sum_y += bps->y[head];
			// 		// printf("%3.3f- head:%d  sum_x:%3.3f  sum_y:%3.3f\n", time, head, sum_x, sum_y);
			// 		head++;
			// 		sum_cnt++;
			// 		if (head >= NBINS_BPS) {
			// 			head = 0;
			...
			// vel_x_temp = (bps->x[tail] - bps->x[head]) / fabs((bps->time[tail] - bps->time[head]));
			// vel_y_temp = (bps->y[tail] - bps->y[head]) / fabs((bps->time[tail] - bps->time[head]));
			vel_x_temp = (bps->x[tail] - bps->x[head]) / (NBINS_BPS);
			vel_y_temp = (bps->y[tail] - bps->y[head]) / (NBINS_BPS);
