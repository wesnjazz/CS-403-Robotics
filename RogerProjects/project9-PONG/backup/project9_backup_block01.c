...
int tt_chase = 1;
int tt_chase_int = 200;
int chase_hold = FALSE;
...
/** base[X,Y,THETA] **/
{
	...
	if (state_search_track == CONVERGED) {	/** if found the red ball **/
		if ( (rec_setp[RS_HOME].base[X] < 0.0 && obs.pos[X] > 0.0) || (rec_setp[RS_HOME].base[Y] > 0.0 && obs.pos[X] < 0.0) ) {
			...
			roger->base_setpoint[X] = rec_setp[RS_HOME].base[X];
			roger->base_setpoint[Y] = rec_setp[RS_HOME].base[Y];
			roger->base_setpoint[THETA] = M_PI + home_angle;
			return TRANSIENT;
		}
