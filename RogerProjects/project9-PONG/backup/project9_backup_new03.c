...
int SEARCH_TRACK_CHASE(Robot* roger, double time)
{
	/***********************************************************************************************/
	/* internal_state=[ 0:SEARCH 1:TRACK ] */
	/***********************************************************************************************/
	...
	internal_state[1] = CHASE(roger, time); // assigns values to rec_setp[1]
	int state = internal_state[0]*3 + internal_state[1];
	printf("%3.3f, SEARCH_TRACK:%d  CHASE:%d  Case:%d\n", time, internal_state[0], internal_state[1], state);
	int return_state;
	switch (state) {
						// SEARCH_TRACK 	CHASE
		case 0: // NO_REFERENCE - NO_REFERENCE
...
int PUNCH(Robot* roger, double time)
{
	if (state_search_track == CONVERGED) {
		...
		if (tiktok_punch % tiktok_punch_interval == 0 && left_touched == FALSE && right_touched == FALSE) {
...
int CHASE_PUNCH(Robot* roger, double time)
{
	...
	switch (state) {
		...
		case 0: // NO_REFERENCE - NO_REFERENCE
			...
			// roger->base_setpoint[Y] = rec_setp[RS_CHASE].base[Y];
			// roger->base_setpoint[THETA] = rec_setp[RS_CHASE].base[THETA];
			// roger->arm_setpoint[LEFT][0] = rec_setp[RS_HOME].arm[LEFT][0];
			// roger->arm_setpoint[LEFT][1] = rec_setp[RS_HOME].arm[LEFT][1];
			// roger->arm_setpoint[RIGHT][0] = rec_setp[RS_HOME].arm[RIGHT][0];
			// roger->arm_setpoint[RIGHT][1] = rec_setp[RS_HOME].arm[RIGHT][1];
			return_state = NO_REFERENCE;
			break;
		case 1: // NO_REFERENCE - TRANSIENT
		case 2: // NO_REFERENCE - CONVERGED
			// roger->base_setpoint[X] = rec_setp[RS_PUNCH].base[X];
			// roger->base_setpoint[Y] = rec_setp[RS_PUNCH].base[Y];
			// roger->base_setpoint[THETA] = rec_setp[RS_PUNCH].base[THETA];
			// roger->arm_setpoint[LEFT][0] = rec_setp[RS_PUNCH].arm[LEFT][0];
			// roger->arm_setpoint[LEFT][1] = rec_setp[RS_PUNCH].arm[LEFT][1];
			// roger->arm_setpoint[RIGHT][0] = rec_setp[RS_PUNCH].arm[RIGHT][0];
			// roger->arm_setpoint[RIGHT][1] = rec_setp[RS_PUNCH].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		case 3: // TRANSIENT - NO_REFERENCE
			roger->base_setpoint[X] = rec_setp[RS_CHASE].base[X];
