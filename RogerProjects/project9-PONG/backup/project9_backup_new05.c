...
{
	rec_setp[RS_RETREAT].arm[LEFT][0] = home_arm[LEFT][0];
	rec_setp[RS_RETREAT].arm[LEFT][1] = home_arm[LEFT][1];
	rec_setp[RS_RETREAT].arm[RIGHT][0] = home_arm[RIGHT][0];
	rec_setp[RS_RETREAT].arm[RIGHT][1] = home_arm[RIGHT][1];
	...
	rec_setp[RS_RETREAT].base[X] = home[X];
	rec_setp[RS_RETREAT].base[Y] = home[Y];
...
int SEARCH_TRACK_CHASE_PUNCH(Robot* roger, double time)
{
	...
	internal_state[1] = CHASE_PUNCH(roger, time); // assigns values to rec_setp[1]
	int state = internal_state[0]*3 + internal_state[1];
	// printf("%3.3f, SEARCH_TRACK:%d  CHASE_PUNCH:%d  Case:%d\n", time, internal_state[0], internal_state[1], state);
	int return_state;
	switch (state) {
