...
int PREDICT(Robot* roger, double time)
{
	...
	if (fabs(t) < 8000.00) {
		...
		// double y_land = 0.0;
		// while (y_predict < MIN_Y || y_predict > MAX_Y) {

		// }
		if (x_predict < 0.0) {
			rec_setp[RS_PREDICT].base[X] = x_predict;
...
int INTERCEPT(Robot* roger, double time)
{
	...
	if (state_predict == CONVERGED) {
		...
		if (ball_LorR == LEFT) {
			if (inv_arm_kinematics_savetotheta(roger, LEFT, obs.pos[X], obs.pos[Y], &theta1, &theta2) == TRUE) {
				rec_setp[RS_INTERCEPT].arm[LEFT][0] = theta1;
				rec_setp[RS_INTERCEPT].arm[LEFT][1] = theta2;
			}
		} else if (ball_LorR == RIGHT) {
			if (inv_arm_kinematics_savetotheta(roger, RIGHT, obs.pos[X], obs.pos[Y], &theta1, &theta2) == TRUE) {
				rec_setp[RS_INTERCEPT].arm[RIGHT][0] = theta1;
				rec_setp[RS_INTERCEPT].arm[RIGHT][1] = theta2;
			}
		}
...
int OBSERVE_PREDICT_INTERCEPT(Robot* roger, double time)
{
	...
	switch (state) {
		...
		case 25:	// CONVERGED -	CONVERGED -			TRANSIENT
			...
			rec_setp[RS_OBSERVE_PREDICT_INTERCEPT].arm[LEFT][0] = rec_setp[RS_INTERCEPT].arm[LEFT][0];
			rec_setp[RS_OBSERVE_PREDICT_INTERCEPT].arm[LEFT][1] = rec_setp[RS_INTERCEPT].arm[LEFT][1];
			rec_setp[RS_OBSERVE_PREDICT_INTERCEPT].arm[RIGHT][0] = rec_setp[RS_INTERCEPT].arm[RIGHT][0];
			rec_setp[RS_OBSERVE_PREDICT_INTERCEPT].arm[RIGHT][1] = rec_setp[RS_INTERCEPT].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
