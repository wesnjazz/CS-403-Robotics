...
#define RS_RETREAT_TURN90_SAMPLE_BACKFORTH_BLOCK		15
...
/** base[X,Y,THETA] **/
{
	...
	if (state_search_track == CONVERGED) {	/** if found the red ball **/
		...
		if (roger->base_position[X] < 0 && roger->base_position[X] > obs.pos[X]) {
			rec_setp[RS_CHASE].base[X] = rec_setp[RS_HOME].base[X];
			return CONVERGED;
		}
		if (roger->base_position[X] > 0 && roger->base_position[X] < obs.pos[X]) {
			rec_setp[RS_CHASE].base[X] = rec_setp[RS_HOME].base[X];
		...
		// double base_offset = 0.8;
		double base_offset = 0.60;  // PONG
		double epsilon = R_BALL + R_BASE + base_offset;
		if (base_err > epsilon) { 
			// if ( (roger->base_position[X] < 0.0 && obs.pos[X] > 0.0) || (roger->base_position[X] > 0.0 && obs.pos[X] < 0.0) ) {
			// 	Roger_mode = ROGER_MODE_DEFENSE;
			// }
			state_chase = TRANSIENT; return TRANSIENT; 
		}
		else { 
...
/** base[X,Y,THETA], arms **/
{
	...
	if (state_chase == CONVERGED && state_punch != CONVERGED && state_retreat != CONVERGED) {
		...
		if (tiktok_punch_nohit % tiktok_punch_interval == 0 && left_touched == FALSE && right_touched == FALSE) {
			printf("\t\t\t\t\t\t[-] NO HIT...\n");
			rec_setp[RS_PUNCH].base[X] = rec_setp[RS_HOME].base[X];
			rec_setp[RS_PUNCH].base[Y] = rec_setp[RS_HOME].base[Y];
...
int SEARCH_TRACK_CHASE_PUNCH(Robot* roger, double time)
{
	/***********************************************************************************************/
	...
	internal_state[1] = CHASE_PUNCH(roger, time);
	int state = internal_state[0]*3 + internal_state[1];
	...
	switch (state) {
		...
		case 6: // CONVERGED - NO_REFERENCE
		case 7: // CONVERGED - TRANSIENT
			roger->eyes_setpoint[LEFT] = rec_setp[RS_CHASE_PUNCH].eye[LEFT];
			roger->eyes_setpoint[RIGHT] = rec_setp[RS_CHASE_PUNCH].eye[RIGHT];
			roger->base_setpoint[THETA] = rec_setp[RS_CHASE_PUNCH].base[THETA];
			roger->base_setpoint[X] = rec_setp[RS_CHASE_PUNCH].base[X];
			roger->base_setpoint[Y] =	rec_setp[RS_CHASE_PUNCH].base[Y];
			roger->arm_setpoint[LEFT][0] = rec_setp[RS_CHASE_PUNCH].arm[LEFT][0];
			roger->arm_setpoint[LEFT][1] = rec_setp[RS_CHASE_PUNCH].arm[LEFT][1];
			roger->arm_setpoint[RIGHT][0] = rec_setp[RS_CHASE_PUNCH].arm[RIGHT][0];
			roger->arm_setpoint[RIGHT][1] = rec_setp[RS_CHASE_PUNCH].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		case 8: // CONVERGED - CONVERGED
			roger->eyes_setpoint[LEFT] = rec_setp[RS_CHASE_PUNCH].eye[LEFT];
...
int TURN90(Robot* roger, double time)
{
	...
	if (fabs(fabs(roger->base_position[THETA]) - fabs(des_angle)) < epsilon) {
		...
		if (rec_setp[RS_HOME].base[X] < 0.0) {
			...
			rec_setp[RS_TURN90].arm[LEFT][0] = -1.57;
			rec_setp[RS_TURN90].arm[LEFT][1] = -2.3;
			rec_setp[RS_TURN90].arm[RIGHT][0] = -0.5;
			rec_setp[RS_TURN90].arm[RIGHT][1] = 2.2;
...
int RETREAT_TURN90(Robot* roger, double time)
{
	int internal_state[2];
	internal_state[1] = RETREAT(roger, time);
	internal_state[0] = TURN90(roger, time);
	int state = internal_state[1]*3 + internal_state[1];
	// printf("%3.3f, RETREAT:%d  TURN90:%d  Case:%d\n", time, internal_state[1], internal_state[0], state);
	int return_state;
	switch (state) {
						// RETREAT 				TURN90
		case 0: // NO_REFERENCE - NO_REFERENCE
		...
		case 4: // TRANSIENT - 		TRANSIENT
		case 5: // TRANSIENT - 		CONVERGED
			rec_setp[RS_RETREAT_TURN90].eye[LEFT] = rec_setp[RS_RETREAT].eye[LEFT];
			rec_setp[RS_RETREAT_TURN90].eye[RIGHT] = rec_setp[RS_RETREAT].eye[RIGHT];
			rec_setp[RS_RETREAT_TURN90].base[THETA] = rec_setp[RS_RETREAT].base[THETA];
			rec_setp[RS_RETREAT_TURN90].base[X] = rec_setp[RS_RETREAT].base[X];
			rec_setp[RS_RETREAT_TURN90].base[Y] = rec_setp[RS_RETREAT].base[Y];
			rec_setp[RS_RETREAT_TURN90].arm[LEFT][0] = rec_setp[RS_RETREAT].arm[LEFT][0];
			rec_setp[RS_RETREAT_TURN90].arm[LEFT][1] = rec_setp[RS_RETREAT].arm[LEFT][1];
			rec_setp[RS_RETREAT_TURN90].arm[RIGHT][0] = rec_setp[RS_RETREAT].arm[RIGHT][0];
			rec_setp[RS_RETREAT_TURN90].arm[RIGHT][1] = rec_setp[RS_RETREAT].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		...
		case 6: // CONVERGED - NO_REFERENCE
			...
			break;
		case 7: // CONVERGED - TRANSIENT
			rec_setp[RS_RETREAT_TURN90].eye[LEFT] = rec_setp[RS_TURN90].eye[LEFT];
			rec_setp[RS_RETREAT_TURN90].eye[RIGHT] = rec_setp[RS_TURN90].eye[RIGHT];
			rec_setp[RS_RETREAT_TURN90].base[THETA] = rec_setp[RS_TURN90].base[THETA];
			rec_setp[RS_RETREAT_TURN90].base[X] = rec_setp[RS_TURN90].base[X];
			rec_setp[RS_RETREAT_TURN90].base[Y] = rec_setp[RS_TURN90].base[Y];
			rec_setp[RS_RETREAT_TURN90].arm[LEFT][0] = rec_setp[RS_TURN90].arm[LEFT][0];
			rec_setp[RS_RETREAT_TURN90].arm[LEFT][1] = rec_setp[RS_TURN90].arm[LEFT][1];
			rec_setp[RS_RETREAT_TURN90].arm[RIGHT][0] = rec_setp[RS_TURN90].arm[RIGHT][0];
			rec_setp[RS_RETREAT_TURN90].arm[RIGHT][1] = rec_setp[RS_TURN90].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		case 8: // CONVERGED - CONVERGED
			rec_setp[RS_RETREAT_TURN90].eye[LEFT] = rec_setp[RS_TURN90].eye[LEFT];
...
int SAMPLE_BACKFORTH(Robot* roger, double time)
{
	...
	switch (state) {
		...
		case 1: // TRANSIENT - 		TRANSIENT
			...
			rec_setp[RS_SAMPLE_BACKFORTH].base[X] = rec_setp[RS_RETREAT_TURN90].base[X];
			rec_setp[RS_SAMPLE_BACKFORTH].base[Y] = rec_setp[RS_BACKFORTH].base[Y];
			rec_setp[RS_SAMPLE_BACKFORTH].base[THETA] = rec_setp[RS_RETREAT_TURN90].base[THETA];
			rec_setp[RS_SAMPLE_BACKFORTH].arm[LEFT][0] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][0];
			rec_setp[RS_SAMPLE_BACKFORTH].arm[LEFT][1] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][1];
			rec_setp[RS_SAMPLE_BACKFORTH].arm[RIGHT][0] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][0];
			rec_setp[RS_SAMPLE_BACKFORTH].arm[RIGHT][1] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		default:
			break;
...
int block_hold = FALSE;
int tt_block = 1;
int tt_block_int = 1500;
int tiktok_block = 1;
int tiktok_block_interval = 100;
int BLOCK(Robot* roger, double time)
{
	tt_block++;
	if(tt_block % tt_block_int == 0) {
		tt_block = 1;
		stereo_observation(roger, &obs);
		// if ( (roger->base_position[X] < 0.0 && obs.pos[X] < 0.0) || (roger->base_position[X] > 0.0 && obs.pos[X] > 0.0) ) {
	...
	if (block_hold) {
		tiktok_block++;
		if (tiktok_block % tiktok_block_interval == 0) {
			tiktok_block = 1;
			block_hold = FALSE;
			return CONVERGED;
		} else {
	...
	int left_reach = inv_arm_kinematics_savetotheta(roger, LEFT, obs.pos[X], obs.pos[Y], &Larm0, &Larm1);
	int right_reach = inv_arm_kinematics_savetotheta(roger, RIGHT, obs.pos[X], obs.pos[Y], &Rarm0, &Rarm1);
	inv_arm_kinematics_savetotheta(roger, LEFT, roger->base_position[X]+1.0, obs.pos[Y], &Larm0, &Larm1);
	inv_arm_kinematics_savetotheta(roger, RIGHT, roger->base_position[X]+1.0, obs.pos[Y], &Rarm0, &Rarm1);
...
int SAMPLE_BACKFORTH_BLOCK(Robot* roger, double time)
{
	...
	switch (state) {
		...
		case 0: // NO_REFERENCE - 			TRANSIENT
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].eye[LEFT] = rec_setp[RS_RETREAT_TURN90].eye[LEFT];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].eye[RIGHT] = rec_setp[RS_RETREAT_TURN90].eye[RIGHT];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[X] = rec_setp[RS_RETREAT_TURN90].base[X];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[Y] = rec_setp[RS_BLOCK].base[Y];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[THETA] = rec_setp[RS_RETREAT_TURN90].base[THETA];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[LEFT][0] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][0];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[LEFT][1] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][1];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[RIGHT][0] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][0];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[RIGHT][1] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		case 1: // TRANSIENT - 					TRANSIENT
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].eye[LEFT] = rec_setp[RS_RETREAT_TURN90].eye[LEFT];
...
int RETREAT_TURN90_SAMPLE_BACKFORTH_BLOCK(Robot* roger, double time)
{
	int internal_state[2];
	internal_state[0] = RETREAT_TURN90(roger, time); // assigns values to rec_setp[0]
	internal_state[1] = SAMPLE_BACKFORTH_BLOCK(roger, time); // assigns values to rec_setp[1]
	int state = internal_state[0]*3 + internal_state[1];
	// printf("%3.3f, RETREAT_TURN90:%d  SAMPLE_BACKFORTH_BLOCK:%d  Case:%d\n", time, internal_state[0], internal_state[1], state);
	int return_state;
	switch (state) {
						// RETREAT_TURN90 	SAMPLE_BACKFORTH_BLOCK
		case 0: // NO_REFERENCE - 	NO_REFERENCE
		case 1: // NO_REFERENCE - 	TRANSIENT
		case 2: // NO_REFERENCE - 	CONVERGED
		case 3: // TRANSIENT - 			NO_REFERENCE
		case 4: // TRANSIENT - 			TRANSIENT
		case 5: // TRANSIENT - 			CONVERGED
			roger->eyes_setpoint[LEFT] = rec_setp[RS_RETREAT_TURN90].eye[LEFT];
			roger->eyes_setpoint[RIGHT] = rec_setp[RS_RETREAT_TURN90].eye[RIGHT];
			roger->base_setpoint[THETA] = rec_setp[RS_RETREAT_TURN90].base[THETA];
			roger->base_setpoint[X] = rec_setp[RS_RETREAT_TURN90].base[X];
			roger->base_setpoint[Y] =	rec_setp[RS_RETREAT_TURN90].base[Y];
			roger->arm_setpoint[LEFT][0] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][0];
			roger->arm_setpoint[LEFT][1] = rec_setp[RS_RETREAT_TURN90].arm[LEFT][1];
			roger->arm_setpoint[RIGHT][0] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][0];
			roger->arm_setpoint[RIGHT][1] = rec_setp[RS_RETREAT_TURN90].arm[RIGHT][1];
			return_state = TRANSIENT;
			break;
		case 6: // CONVERGED - 			NO_REFERENCE
		case 7: // CONVERGED - 			TRANSIENT
...
// int tt_pong = 1;
// int tt_pong_int = 1500;
int PONG(Robot* roger, double time)
{
	// tt_pong++;
	// if (tt_pong % tt_pong_int == 0) {
	// 	if (Roger_mode == ROGER_MODE_OFFENSE) {
	// 		Roger_mode = ROGER_MODE_DEFENSE;
	// 	} else {
	// 		Roger_mode = ROGER_MODE_OFFENSE;
	// 	}
	...
	// if (time > 0.0000 && time < 0.010) {
	// 	if ( (roger->base_setpoint[X] < 0.0 && obs.pos[X] > 0.0) || (roger->base_position[X] > 0.0 && obs.pos[X] < 0.0) ) {
	// 		Roger_mode = ROGER_MODE_DEFENSE;
	// 	} else {
	// 		// Roger_mode = ROGER_MODE_OFFENSE;
	// 		;
	...
	if (Roger_mode == ROGER_MODE_DEFENSE) {
		// printf("  DEFENSE\n");
		RETREAT_TURN90_SAMPLE_BACKFORTH_BLOCK(roger, time);
	} else if (Roger_mode == ROGER_MODE_OFFENSE) {
		// printf("  OFFENSE\n");
		SEARCH_TRACK_CHASE_PUNCH(roger, time);
	}
}
...
double time;
{
	if (time < 0.0010) {
		...
		rec_setp[RS_HOME].arm[LEFT][1] = -2.6;
		rec_setp[RS_HOME].arm[RIGHT][0] = -2.0;
		rec_setp[RS_HOME].arm[RIGHT][1] = 2.6;
