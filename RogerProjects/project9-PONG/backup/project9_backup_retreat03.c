...
/** base[X,Y,THETA] **/
{
	...
	if (state_search_track == CONVERGED) {	/** if found the red ball **/
		...
		if (fabs(roger->base_velocity[X]) > 5.0 || fabs(roger->base_velocity[Y] > 5.0) ) {
			printf("\t\t\t\t\t\t\t\tFAST 5.0!\n");
			base_offset = 0.9;
		}

		if (fabs(roger->base_velocity[X]) > 9.0 || fabs(roger->base_velocity[Y] > 9.0) ) {
			printf("\t\t\t\t\t\t\t\tFAST 9.0!\n");
			base_offset = 1.2;
		}
...
/** base[X,Y,THETA], arms **/
{
	...
	if (state_chase == CONVERGED && state_punch != CONVERGED && state_retreat != CONVERGED) {
		...
		if ((roger->base_position[X] < 0.0 && roger->base_position[THETA] > 0.0) || (roger->base_position[X] >= 0.0 && roger->base_position[THETA] < 0.0))  {
		tiktok_punch_nohit++;
			...
			rec_setp[RS_PUNCH].arm[LEFT][1] = -1.8;
			rec_setp[RS_PUNCH].arm[RIGHT][0] = rec_setp[RS_HOME].arm[RIGHT][0];
			rec_setp[RS_PUNCH].arm[RIGHT][1] = rec_setp[RS_HOME].arm[RIGHT][1];
		} else if ((roger->base_position[X] < 0.0 && roger->base_position[THETA] < 0.0) || (roger->base_position[X] >= 0.0 && roger->base_position[THETA] >= 0.0)) {
		tiktok_punch_nohit++;
			rec_setp[RS_PUNCH].base[X] = obs.pos[X];
			rec_setp[RS_PUNCH].base[Y] = obs.pos[Y];
			rec_setp[RS_PUNCH].base[THETA] = roger->base_position[THETA] + M_PI/2.0;
			...
			rec_setp[RS_PUNCH].arm[RIGHT][0] = rec_setp[RS_HOME].arm[RIGHT][0];
			rec_setp[RS_PUNCH].arm[RIGHT][1] = 1.8;
		} else {
		tiktok_punch_nohit++;
			rec_setp[RS_PUNCH].base[X] = obs.pos[X];
			rec_setp[RS_PUNCH].base[Y] = obs.pos[Y];
...
int PONG(Robot* roger, double time)
{
	...
	// }
		...
		// printf("%3.3f- base_vel(%3.3f,%3.3f,%3.3f)\n", time, roger->base_velocity[X], roger->base_velocity[Y], roger->base_velocity[THETA]);
