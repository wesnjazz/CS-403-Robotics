...
#define RS_BLOCK														11
#define RS_SAMPLE_BACKFORTH_BLOCK						12

#define RS_TURN90_SAMPLE_BACKFORTH_BLOCK		13
...
double last_position = MAX_Y;
int tiktok_sample = 1;
int tiktok_sample_interval = 50;
double ul_1st = -1.0;
double ul_2nd = -1.0;
#define DIR_FORTH 0
#define DIR_BACK	1
		...
		if (sampled_two != TRUE) {
			...
			if (tiktok_sample % tiktok_sample_interval == 0 && ul_1st >= 0.0) {
				...
				if (diff > 0.0) { direction = DIR_FORTH; } /** direction is FORTH: ul change is positive: from right to left on the eye image plane **/
					...
					if (ul_1st > 63.5) { direction = DIR_BACK; }	/** red pixel is on the right on the eye **/
					else { direction = DIR_FORTH; }	/** red pixel is on the left on the eye **/
				}
...
int SAMPLE_BACKFORTH_BLOCK(Robot* roger, double time)
{
	...
	// printf("%3.3f, SAMPLE:%d  BACKFORTH:%d  BLOCK:%d  Case:%d\n", time, internal_state[0], internal_state[1], internal_state[2], state);
	int return_state;

	switch (state) {
							// SAMPLE 				BACKFORTH 			BLOCK
		...
		case 9:		// TRANSIENT - 		NO_REFERENCE -	NO_REFENRECE
		case 10:	// TRANSIENT - 		NO_REFERENCE -	TRANSIENT
		case 11:	// TRANSIENT - 		NO_REFERENCE -	CONVERGED
		case 12:	// TRANSIENT - 		TRANSIENT -			NO_REFENRECE
			...
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[Y] = rec_setp[RS_BACKFORTH].base[Y];
		...
		case 15:	// TRANSIENT - 		CONVERGED -			NO_REFENRECE
		case 16:	// TRANSIENT - 		CONVERGED -			TRANSIENT
		case 17:	// TRANSIENT - 		CONVERGED -			CONVERGED
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].eye[LEFT] = rec_setp[RS_TURN90].eye[LEFT];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].eye[RIGHT] = rec_setp[RS_TURN90].eye[RIGHT];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[X] = rec_setp[RS_TURN90].base[X];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[Y] = rec_setp[RS_TURN90].base[Y];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].base[THETA] = rec_setp[RS_TURN90].base[THETA];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[LEFT][0] = rec_setp[RS_TURN90].arm[LEFT][0];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[LEFT][1] = rec_setp[RS_TURN90].arm[LEFT][1];
			rec_setp[RS_SAMPLE_BACKFORTH_BLOCK].arm[RIGHT][0] = rec_setp[RS_TURN90].arm[RIGHT][0];
