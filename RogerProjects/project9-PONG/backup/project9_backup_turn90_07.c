...
int SAMPLE_BACKFORTH(Robot* roger, double time)
{
	...
	switch (state) {
		...
		case 1: // TRANSIENT - 		TRANSIENT
			rec_setp[RS_SAMPLE_BACKFORTH].eye[LEFT] = rec_setp[RS_TURN90].eye[LEFT];
			rec_setp[RS_SAMPLE_BACKFORTH].eye[RIGHT] = rec_setp[RS_TURN90].eye[RIGHT];
...
int BLOCK(Robot* roger, double time)
{
	...
	// if (right_reach) {
	// 	// printf("\t\t\t\t\t\tRight REACH\n");
	// 	rec_setp[RS_BLOCK].base[Y] = obs.pos[Y];
	// 	rec_setp[RS_BLOCK].arm[RIGHT][0] = Rarm0;
	// 	rec_setp[RS_BLOCK].arm[RIGHT][1] = Rarm1;
	// }
