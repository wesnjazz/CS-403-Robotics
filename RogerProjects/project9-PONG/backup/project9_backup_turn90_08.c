...
/** base[X,Y,THETA] **/
{
	...
	if (state_search_track == CONVERGED) {	/** if found the red ball **/
		...
		if ( (rec_setp[RS_HOME].base[X] < 0.0 && obs.pos[X] > 0.0) || (rec_setp[RS_HOME].base[Y] > 0.0 && obs.pos[X] < 0.0) ) {
			tiktok_chase++;
			if (tiktok_chase % tiktok_chase_interval == 0) {
				printf("\t\t\t\t\t\t\t\tSwitch Mode\n");
				if (Roger_mode == ROGER_MODE_OFFENSE) { Roger_mode = ROGER_MODE_DEFENSE; }
				else { Roger_mode = ROGER_MODE_OFFENSE; }
				tiktok_chase = 1;
			}
		}
		// 	printf("%3.3f- \t\t\t\t\t\t\t\tRed Ball is in the opposite side!\n");
...
int BLOCK(Robot* roger, double time)
{
	...
	if (-M_PI/2.0 <= roger->base_position[THETA] && roger->base_position[THETA] <= M_PI/2.0) {
		if (obs.pos[Y] > roger->base_position[Y]) {
			inv_arm_kinematics_savetotheta(roger, LEFT, roger->base_position[X]-0.2, obs.pos[Y]-0.2, &Larm0, &Larm1);
		...
		if (obs.pos[Y] > roger->base_position[Y]) {
			inv_arm_kinematics_savetotheta(roger, RIGHT, roger->base_position[X]-0.2, obs.pos[Y]-0.2, &Rarm0, &Rarm1);
