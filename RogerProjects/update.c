...
Observation obs;
{
  // printf("draw_observation in update.c\n");
  int i, j;
...
int button;
{
  ...
  // 0.75 meters directly in front of the robot (in the x_b direction)
  ...
  // 0.75 meters to the left of the robot (in the y_b direction)
  // roger->base_setpoint[X] = 0.0;
      ...
      case JOINT_ANGLE_INPUT:
        ...
        if (roger->control_mode == PROJECT1) { // GUI inputs to Motor Units
          ...
          if (roger->button_event == LEFT_BUTTON) {
            ...
            // roger->arm_setpoint[body_side][0] = 1.0;
            // roger->arm_setpoint[body_side][1] = -1.0;
          }
